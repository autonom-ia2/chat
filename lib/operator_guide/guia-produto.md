...
- intent: Como acho empresas de um segmento na minha cidade?; Como priorizo quem ligar primeiro?; Como viro esse lead em contato?; Dá para criar card no CRM a partir da busca?; O que é a jogada da busca?; Onde ficam os filtros avançados da busca?; O que significa o selo de modo no topo da busca?; Por que a jogada desmarcou sozinha?; Como desenho a área da busca no mapa?; Como repito uma busca do histórico?; Quantos leads uma busca traz?; Como ordeno os leads por distância ou posição no Google?; Por que o lead está em Enriquecendo?; O que são os números e os grupos de pinos no mapa?
...
- passos: 1. Abra Prospecção > Buscar leads e clique em nova busca; 2. Escreva o segmento e a localização e escolha uma sugestão, que vem só do país da busca da conta; 3. Defina a área (raio, área visível, ou um círculo, retângulo ou polígono desenhado no mapa), o limite, até 60, e o tipo de decisor (por enquanto só Proprietário); 4. Escolha a forma de pesquisa e, se quiser, uma jogada, que preenche os filtros avançados; 5. Ajuste os filtros avançados na gaveta de quatro grupos e clique em Aplicar; 6. Busque; 7. No botão Filtros dos resultados, escolha a ordem (prioridade, score, avaliação, número de avaliações, distância do centro, posição no Google, data ou nome), inverta com a seta e refine com Aplicar; 8. Clique no card de um lead para abrir os detalhes, com reputação e fatores de atenção; 9. Use criar card, que já cria o contato junto, ou selecione vários e crie os cards em lote; 10. No histórico, use Repetir para rodar a mesma busca na hora ou Editar e buscar para abrir o formulário preenchido.
- gotchas: a busca lê até três páginas de 20 no Google e traz até 60 leads, filtrando antes de cortar, e cada lead guarda a posição real no Google; o Google às vezes acaba antes de 60; não há limite diário nem mensal, mas cada página lida conta uma unidade no consumo; com área desenhada, o botão Buscar só libera com o desenho pronto (polígono com três pontos ou mais), e trocar o tipo de área apaga o desenho; retângulo e polígono só trazem empresas de dentro do desenho, o círculo é só preferência e pode trazer quem está fora; o local digitado vai junto com o termo, então desenhar longe dele pode dar zero resultado; Repetir e Editar e buscar restauram tudo, inclusive o desenho e a ordem, e dentro do Cache TTL (24 horas se ninguém mudou) a mesma busca volta do cache sem consultar o Google; com a pesquisa liberada, os leads nunca enriquecidos são enriquecidos em segundo plano depois da busca, o card mostra Enriquecendo enquanto o lead está na fila e se atualiza sozinho quando termina, sem recarregar; enriquecimento que falha não apaga o que já tinha sido achado e o botão volta a Enriquecer; preso mais de 15 minutos, o lead é dado como interrompido e volta a aceitar pedido; a ordem por distância só aparece em busca com centro gravado; o número do pino no mapa é a Posição do lead na fila, a mesma do card, e pinos próximos se juntam num grupo com a quantidade; o card mostra bairro, até quatro sinais (site, fone, fotos, posição no Google, e nota ou avaliações se sobrar vaga), telefone no formato internacional com selo de verificado e botões de Instagram, Facebook e LinkedIn quando existem; sem a chave de plataforma configurada pelo suporte, as sugestões não aparecem e a busca não roda; o mapa usa uma segunda chave de plataforma e, sem ela, a busca funciona mas o mapa não abre; o botão Enriquecer só libera com a pesquisa liberada pelo suporte; sem a chave de IA da integração CRM Kanban AI, a tela avisa e o enriquecimento traz só o que está no site; criar card exige um funil ativo configurado; excluir uma busca do histórico não apaga os leads já salvos; o selo no topo mostra o modo, GMN (atacar lacunas) ou Geral (qualificar leads), da nova busca ou da busca aberta; são três jogadas por modo mais Sem jogada, que sempre volta os filtros ao padrão; a jogada desmarca sozinha quando um filtro fica diferente dela, e trocar a forma de pesquisa com a jogada marcada tira a jogada e os filtros dela; o topo da gaveta de filtros mostra a jogada base e o modo; filtro só vale depois de Aplicar, e fechar a gaveta sem aplicar descarta o rascunho; Aberto agora e Tem horário só filtram por sim; a faixa de posição vai de 1 a 40, e com a alça da direita no fim entram também as posições até a 60ª; cortar as primeiras N posições deixa no máximo 60 menos N empresas; a busca guarda a posição de cada lead nela, e reabrir uma busca antiga mostra a posição daquela busca; erro do Google aparece em português com o que fazer, e o erro de local fica abaixo do campo Localização; os telefones são lidos no formato do país da busca, e número que não vale nele nem como internacional fica sem link de ligar e de WhatsApp.
...
### Ajustar funil padrão, país da busca, cache e critérios de score da prospecção
