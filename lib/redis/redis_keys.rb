module Redis::RedisKeys
  ...
  # Serializes WhatsApp Coexistence history-import processing per channel: Meta can redeliver the
  # same history chunk concurrently, and the per-message `Message.find_by(source_id:)` dedupe check
  # alone is not atomic against a concurrent duplicate insert.
  WHATSAPP_HISTORY_SYNC_MUTEX = 'WHATSAPP_HISTORY_SYNC_LOCK::%<inbox_id>s'.freeze
