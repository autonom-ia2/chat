...
RSpec.describe Autonomia::Agents::Tools::Native::Base do
  ...
  describe 'ver_resultado_da_cotacao' do
    ...
    # A lista fechada (chat#718): seguradoras é uma lista de nomes, um por item, e não um texto que o código quebraria.
    it 'declara seguradoras (lista ou null) e produto (string ou null), presentes em required, sem anyOf, e nenhuma outra' do
      ...
      expect(schema[:parameters][:properties]['seguradoras']).not_to have_key('anyOf')
