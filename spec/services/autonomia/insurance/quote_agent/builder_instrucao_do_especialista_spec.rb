...
module ManualDoEspecialistaDeAuto
  ...
  PROMESSAS = {
    ...
    # O ERRO QUE A FRASE EVITA (20/09/2026, conversa 6982): o cliente deu nome, CPF, nascimento e sexo de
    # quem dirige, o especialista os escreveu no SEGURADO e deixou o condutor vazio, e a trava do adapter
    # recusou com "empresa não dirige". A âncora prova que os campos do condutor existem, separados dos
    # do segurado, e que a razão social não precisa ser perguntada (o nome do segurado é opcional).
...
RSpec.describe Autonomia::Insurance::QuoteAgent::Builder do
  ...
  # Pela chat#638 (`bd633e21…` -> `abbed8b0…`): o CPF da pessoa indicada vem dos documentos e cotações da conversa antes
  # de ser pedido.
  it 'o manual do ramo é o texto revisado — mudou? revise PROMESSAS e assine aqui' do
    expect(Digest::MD5.hexdigest(ManualDoEspecialistaDeAuto::ARQUIVO.binread)).to eq('abbed8b0703a22842e2a83e7f497f8f2')
  end

  # chat#612 (23/09/2026): a §G e a §J não contam recusa nem motivo ao cliente; só a instabilidade.
  # chat#638 (24/09/2026, `1fface70…` -> `d08359f3…`): nem a instabilidade nem o prazo; o motivo fica com a equipe.
  it 'o bloco comum é o texto revisado — mudou? revise PROMESSAS e assine aqui' do
    expect(Digest::MD5.hexdigest(ManualDoEspecialistaDeAuto::BLOCO_COMUM.binread)).to eq('d08359f3f07e358f9923cb5c30d19a37')
  end
