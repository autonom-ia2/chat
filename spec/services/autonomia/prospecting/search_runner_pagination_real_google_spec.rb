...
RSpec.describe Autonomia::Prospecting::SearchRunner do
  ...
  # Desde o #681 a busca guarda também a nota do Orth em sombra, ao lado da legada, sem mudar a legada.
  it 'guarda nota, detalhe da nota e prioridade de cada lead na própria busca' do
    ...
    expect(scoring[lead.id.to_s].except('orth')).to eq(
    ...
    expect(scoring.values.map { |item| item['orth'].keys }).to all(match_array(%w[score priority_score band human_insight]))
  end
