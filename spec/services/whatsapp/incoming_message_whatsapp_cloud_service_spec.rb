  ...
  describe '#perform' do
    ...
    context 'when WhatsApp Flow nfm_reply is received' do
      ...
      it 'preserves the raw flow response in content attributes and sets a safe content placeholder' do
        ...
        # Since Chatwoot 4.17 the core sets the i18n placeholder itself; the fork patch only
        # guarantees the webhook-facing `flow_response` attribute below.
        expect(message.content).to eq(I18n.t('conversations.messages.whatsapp.flow_response'))
