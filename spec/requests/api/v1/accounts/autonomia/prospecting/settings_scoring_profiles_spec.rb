...
RSpec.describe 'Autonomia prospecting settings: perfis de nota', type: :request do
  ...
  describe 'motor de nota' do
    ...
    context 'with a conta virada para o Orth' do
      ...
      # Mapeamento do contrato da E5 (WeightMapping, frente A): google_rank e query_relevance saem e o resto
      # vai para 100. O dublê fixa só o contrato; a conta da frente A é testada lá.
      let(:weight_mapping) do
        Module.new do
          def self.from_legacy(weights)
            mapped = { 'website' => weights['website'], 'phone' => weights['phone'], 'rating' => weights['rating'],
                       'volume' => weights['reviews_count'], 'activity' => weights['activity'], 'photos' => weights['photos'] }
            total = mapped.values.sum.to_f
            mapped.transform_values { |value| (value * 100 / total).round }
          end
        end
      end

      before do
        stub_const('Autonomia::Prospecting::Scoring::WeightMapping', weight_mapping)
        stub_const('Autonomia::Prospecting::Scoring::OrthScorer::DEFAULT_WEIGHTS', orth_default)
        setting = Autonomia::Prospecting::Setting.for_account(account)
        setting.update!(metadata: setting.metadata.to_h.merge('score_engine' => 'orth'))
      end
