...
# blob já gravado e o token da entrega. A URL do portal não está nela. A legenda saiu na PR C: o arquivo sai
# sozinho, e a forma adiada por uma versão anterior ainda tem a chave, que é ignorada.
#
...
class Autonomia::Agents::Tools::ArquivoGravado
  ...
  attr_reader :blob_assinado, :token
  ...
  def self.de(valor)
    ...
    new(blob_assinado: forma['blob'], token: forma['token'])
  end

  def initialize(blob_assinado:, token:)
  ...
  def valida_para?(run)
    blob_assinado.present? && token.match(TOKEN)&.[](:execucao) == run.execution_key
  end
  ...
  def to_h
    { CHAVE => { 'blob' => blob_assinado, 'token' => token } }
  end
end
