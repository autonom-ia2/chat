# Avalia em memória a árvore de condições de público-alvo do agente Autonom.ia contra o contato da
# conversa (#284 · Entrega 2a). Cópia do desenho do Captain::AudienceMatcher (sem dependência de
# Captain): um nó é grupo ({ operator:, conditions: [...] }) ou folha ({ attribute_key:,
# filter_operator:, values: [...] }). A semântica dos operadores espelha Contacts::FilterService
# para que o público case com os mesmos contatos dos segmentos.
class Autonomia::Agents::AudienceMatcher
  ...
  # labels é "tem a etiqueta", booleanos convertem a string esperada, telefones ignoram o "+" e
  # texto compara sem diferenciar maiúsculas.
  ...
  # Checkbox não definido conta como false.
  ...
  # -1/0/1 como <=>, ou nil quando vazio/não-parseável (nunca casa).
  ...
  # Atributos de data personalizados ficam como string ISO no jsonb; tratamos como data igual ao
  # Contacts::FilterService (que converte no SQL).
