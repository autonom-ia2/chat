...
// #284 (Entrega 2a) — attributes the agent audience can target: contact standard/custom
// attributes (same list as contact segments) plus two conversation attributes. Mirrors the
// Captain audience picker with the fork's own i18n keys (pt_BR parity).
...
export function useAgentAudienceFilterTypes() {
  const { t } = useI18n();
  ...
  const conversationTypes = computed(() => [
    conversationOption('hmac_verified', t('AGENTS.AUDIENCE.LOGGED_IN'), [
      { id: 'true', name: t('AGENTS.AUDIENCE.LOGGED_IN_TRUE') },
      { id: 'false', name: t('AGENTS.AUDIENCE.LOGGED_IN_FALSE') },
    ]),
    conversationOption(
      'browser_language',
      t('AGENTS.AUDIENCE.BROWSER_LANGUAGE'),
      languages
    ),
  ]);
  ...
  const filterTypes = computed(() => [
    header('contact', t('AGENTS.AUDIENCE.GROUP_CONTACT')),
    ...standardTypes.value,
    header('conversation', t('AGENTS.AUDIENCE.GROUP_CONVERSATION')),
    ...conversationTypes.value,
    ...(customTypes.value.length
      ? [
          header('custom', t('AGENTS.AUDIENCE.GROUP_CUSTOM')),
          ...customTypes.value,
        ]
      : []),
  ]);
