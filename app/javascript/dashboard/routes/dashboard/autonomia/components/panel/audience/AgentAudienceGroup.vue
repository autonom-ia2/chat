...
// #284 (Entrega 2a) — one and/or group of the agent audience tree. Recursive: a
// child is either a leaf (ConditionRow) or another group (one level deep).
...
const nextId = () => {
  uid += 1;
  return `agent-audience-${Date.now()}-${uid}`;
};
...
const operatorOptions = [
  { value: 'and', label: t('AGENTS.AUDIENCE.MATCH_ALL') },
  { value: 'or', label: t('AGENTS.AUDIENCE.MATCH_ANY') },
];
...
const removeChild = index => {
  ...
  // A nested group must always hold at least one condition; drop the group when emptied.
  ...
  >
    <div class="flex items-center gap-2">
      <div class="flex items-center gap-1.5 text-sm text-n-slate-11">
        <span>{{ t('AGENTS.AUDIENCE.MATCH_PREFIX') }}</span>
        <FilterSelect
        ...
        />
        <span>{{ t('AGENTS.AUDIENCE.MATCH_SUFFIX') }}</span>
      </div>
      <Button
        v-if="!isRoot"
        sm
    ...
    >
      ...
      <p class="text-sm font-medium text-n-slate-12">
        {{ t('AGENTS.AUDIENCE.EMPTY_TITLE') }}
      </p>
      <p class="max-w-md text-sm text-n-slate-11">
        {{ t('AGENTS.AUDIENCE.EMPTY_BODY') }}
      </p>
    </div>
    ...
    <div class="flex gap-2">
      <Button
        ...
        icon="i-lucide-plus"
        class="flex-shrink-0"
        :label="t('AGENTS.AUDIENCE.ADD_CONDITION')"
        @click="addCondition"
      />
      <Button
        v-if="depth < maxDepth"
        ...
        icon="i-lucide-plus"
        class="flex-shrink-0"
        :label="t('AGENTS.AUDIENCE.ADD_GROUP')"
        @click="addGroup"
      />
    </div>
  </div>
