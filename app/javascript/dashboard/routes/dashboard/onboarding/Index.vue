...
// Pre-fill the editable company name from the account. Only fills when empty
// so a late-arriving account payload never clobbers what the user typed.
watch(
  accountName,
  name => {
    if (!companyName.value && name) companyName.value = name;
  },
  { immediate: true }
);
