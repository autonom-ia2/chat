...
class Integrations::Hook < ApplicationRecord
  ...
  before_validation :enable_crm_kanban_ai_by_default, on: :create, if: :crm_kanban_ai?
  ...
  # Só na criação: quem desligou a IA depois, de propósito, continua desligado.
  def enable_crm_kanban_ai_by_default
    self.settings = settings.to_h.merge('enabled' => true) if settings.to_h['enabled'].nil?
  end
