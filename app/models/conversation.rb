...
#  idx_conversations_campaign_source_ids_trgm           (((additional_attributes ->> 'campaign_source_ids'::text)) gin_trgm_ops) USING gin
...
class Conversation < ApplicationRecord
  ...
  def handle_campaign_status
    # If campaign has no sender (bot-initiated) and inbox has an active bot, let the bot handle it.
  ...
  def set_active_bot_conversation
    ...
    # nativo atende conversa SEM responsável em qualquer status — então não força `pending` nessas
    # caixas. Bots NÃO-webhook (ex.: Captain/Dialogflow, que exigem pending) seguem nascendo pending.
    # TODO (upstream): make this an inbox config instead of assuming bot conversations should start as pending
    self.status = :pending unless webhook_bot_inbox?
