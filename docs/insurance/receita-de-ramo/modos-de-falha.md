...
As falhas que custaram caro na construção de auto (03 a 21/09/2026) e de residencial (21 a 24/09/2026). **Leia antes de escrever a
...
**B17. Abrir várias cotações no mesmo segundo bloqueia a conta.** Quatro aberturas simultâneas nasceram com 3, 2,
4 e 1 seguradoras, disjuntas; minutos depois o firewall do portal devolveu "Access Denied" em HTML dentro do
envelope, lido como senha errada, e a conta parou. *Pega:* abertura uma por vez por conexão (chat#621), memória do
login válido e HTML como `unavailable` (adapters#94). *Outro ramo:* sim, e vale para a medição em lote. *Fonte:*
MEM `agger-sessoes-paralelas-funcionam.md` (23/09). *Origem:* produção.

**B18. Recusa escrita só dentro do plano.** O portal escreve a recusa em `calculos[].resultados[].erros`, às vezes
só ali; o adapter só olhava `calculos[].erros`, e a oferta saía `error` sem motivo: com as quatro seguradoras
recusando por escrito, a Lia disse que não conseguiu concluir. *Pega:* `agger-quote-fluxo.test.ts` (adapters#97).
*Outro ramo:* sim. *Fonte:* cotação QUU0I17, 23/09. *Origem:* produção.
...
**C12. CPF perguntado com o documento na mão.** O especialista residencial tinha a apólice do Valter, com o CPF, e o
cliente tinha dito "a casa dele"; perguntou o CPF assim mesmo. A regra "a apólice de outra pessoa nunca dá o
segurado" não separava o documento que escolhe o segurado do que traz o CPF de quem o cliente nomeou. *Pega:* a
regra reescrita e assinada (chat#639); não há teste de comportamento. *Outro ramo:* sim. *Fonte:* conversa 7150,
24/09. *Origem:* produção.

**C13. Histórico de outra rodada decide o dado.** "O mesmo CPF da cotação do carro", com dois carros cotados na
conversa (um da véspera), fez o especialista abrir a cotação com o CPF errado e corrigir no mesmo turno (execuções
93 e 94). O supersede protegeu: só a segunda foi ao portal. *Pega:* resolver a conversa entre rodadas de teste; a
regra de C12. *Outro ramo:* sim. *Fonte:* conversa 7150, 24/09. *Origem:* produção.
...
rodadas furadas. *Pega:* **resolvido por outro caminho**: desde a chat#634 nenhum motivo vai ao modelo, a lista de
palavras saiu, e o motivo vai para a equipe numa nota interna. *Outro ramo:* sim. *Fonte:* F2:258-277, chat#634.
*Origem:* teste.

**D13. A frase que não se quer ouvir estava no texto que o modelo lê.** "Algumas seguradoras não responderam a
tempo, mas isso não significa recusa do risco" saiu três vezes numa rodada, em auto e residencial: era quase a
letra do fato `encerrada_por_prazo`. *Pega:* `insurance_quote_nenhum_estado_mudo_spec.rb` (nenhum desfecho fala de
prazo ou recusa), assinaturas das instruções (chat#639). *Outro ramo:* sim. *Fonte:* conversas 7057 e 7150, 24/09.
*Origem:* produção.
