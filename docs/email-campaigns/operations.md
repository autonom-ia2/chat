...
Antes de pedir aprovação de merge, cumprir e registrar todos os [gates adversariais da release](release-436.md#gates-adversariais-obrigatórios-antes-de-merge): locks entre versão implantada/candidata, bloqueio do provedor versus claim nas duas ordens, progresso sob feedback contínuo, renderização HTTP real da importação, ordem de chegada da quarentena, correção com a mesma chave, denominador misto e erro aninhado na UI. Esses gates foram executados localmente após os rebases e permanecem obrigatórios no CI/review do SHA publicado. Em cada merge, a compatibilidade da versão efetivamente implantada ainda deve ser conferida antes de prosseguir para a próxima PR.
