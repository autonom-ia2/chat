...
**Handoff adversarial da PR442:** o parent rebaseia sobre PR441 corrigida e traz de PR438 a promoção de evidência corrigida com a mesma chave. A regressão ativa em `spec/services/email_campaigns/maintenance/corrected_evidence_spec.rb` depende desse contrato. O registry desta base retorna duplicata antes de aplicar a correção; isso foi identificado por leitura, sem executar Rails nesta rodada. Não contornar a dependência mudando a chave em Evidence nem duplicando o registry na manutenção. Sintaxe/RuboCop não estabelecem aprovação de merge.
...
Antes de pedir aprovação de merge, cumprir e registrar todos os [gates adversariais da release](release-436.md#gates-adversariais-obrigatórios-antes-de-merge): locks entre versão implantada/candidata, bloqueio do provedor versus claim nas duas ordens, progresso sob feedback contínuo, renderização HTTP real da importação, ordem de chegada da quarentena, correção com a mesma chave, denominador misto e erro aninhado na UI. São pendências após os rebases, não resultados desta rodada. Se a ordem de locks entre versões for incompatível, não aprovar blue-green misto com base apenas nos testes da candidata; o plano de transição precisa ser revisto e aprovado separadamente.
