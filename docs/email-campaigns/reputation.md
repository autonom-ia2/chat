...
Integração final da #443 revisada em 2026-09-17. O review externo reabriu a pausa superseded em `shadow/warning` e a perda de uma observação nociva do provider fora de ordem; ambas foram corrigidas e exercitadas com conexões PostgreSQL reais em ambiente descartável. **Isto não autoriza merge/deploy**: o novo HEAD ainda exige CI próprio e nova revisão independente. Evidência de fechamento em [2026-09-17-pr443-external-review-fixes.md](../audit/2026-09-17-pr443-external-review-fixes.md).
...
**Integração final:** o contrato foi propagado aos relatórios, backfill e contadores derivados. `EmailCampaign#event_counters` usa `REAL_COMPLAINT_SQL`, de modo que prevenção OnAccount/OnTenant não incrementa `complained_count`. O contador `sent_count` usa `sent_at` como evidência durável de aceite: uma prevenção posterior pode mudar o status atual para `suppressed`, mas não apaga um envio já aceito. As regressões SNS, métricas e apresentação fazem parte do aggregate final; nenhum enum ou API externa foi alterado para obter esse resultado.
...
As migrations ainda não foram implantadas em produção. O fechamento da #443 executou `main schema → db:migrate` em PostgreSQL 17 descartável, incluindo a criação de `harmful_generation` no estado global do provider e os guards de histórico; não houve acesso a banco de produção.

## Concorrência, custo e fila

`Observation` reserva uma geração monotônica em lock curto de estado; coleta métricas e fingerprint **fora** de locks de conta/estado. Publicação adquire account → state e verifica geração e versão de feedback. Observações ultrapassadas retornam `reputation_evaluation_superseded` e nunca autorizam retomada/override nem publicam métricas como atuais. Uma observação superseded pode somente **adicionar proteção**: em `shadow/warning`, a decisão efetiva usa `LegacyDecision`; em `enforce`, usa a policy classificada. O snapshot imutável guarda as métricas observadas, política, geração, versão observada de feedback, `superseded=true`, `policy_pause` e a decisão efetiva `pause`; não altera `current_metrics`, `policy`, `evaluated_at` nem `evaluated_feedback_version` e não substitui incidente já bloqueado. Portanto, `current_metrics` continua sendo a última avaliação publicada, enquanto o snapshot explica o bloqueio conservador. Regressões com três ciclos consecutivos em `shadow`, `warning` e `enforce` demonstram que feedback contínuo não mantém o próximo claim aberto.
...
Após bloqueio, uma leitura saudável apenas torna a condição elegível para revisão: `status=healthy` pode coexistir com `blocked=true`. Monitor não libera latch. Polls sobrepostos não podem fazer telemetria antiga substituir `status/telemetry/checked_at` mais novos; porém uma resposta nociva tardia ainda incrementa `harmful_generation`, adiciona `blocked=true` e audita a primeira transição de latch. Erro/`unknown` preservado sobre um estado já bloqueado **não** incrementa essa geração. A liberação exige `ProviderRelease`, SuperAdmin real, motivo e **nova consulta** com sending habilitado, enforcement HEALTHY e ambas as taxas frescas abaixo dos limiares. O release captura `harmful_generation` antes de iniciar a nova coleta externa e exige a mesma geração sob o lock final; qualquer observação nociva persistida durante a rechecagem — inclusive a própria resposta nociva terminando fora de ordem — invalida a liberação. Registra `provider_released` somente após esse fence. Uma nova transição a bloqueio registra `provider_blocked`, sem payload bruto do provedor. Erros persistem só classe, sem mensagem privada.
