...
O GitHub rejeitou o uso do contexto `runner` no env de nível de job. A configuração final usa um passo anterior à instalação que grava o caminho em `GITHUB_ENV` a partir de `RUNNER_TEMP`, tornando a mesma variável disponível à instalação e ao harness. A existência do executável continua sendo verificada. A sintaxe YAML isolada não valida disponibilidade de contextos GitHub; o gate remoto permanece obrigatório. Referência: documentação oficial de contextos e variáveis do GitHub Actions.
