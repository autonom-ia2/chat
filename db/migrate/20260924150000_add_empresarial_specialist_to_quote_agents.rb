# O ESPECIALISTA DE EMPRESARIAL NOS AGENTES QUE JÁ EXISTEM (chat#641, 24/09/2026), no molde do de residencial.
#
# O `Builder` cria os especialistas só no nascimento do agente (`JaExiste` depois). Sem isto, o especialista de
# empresarial valeria só para agente criado a partir deste deploy, e o agente em produção seguiria sem ele. O
...
# `ON CONFLICT` o respeita). E não liga empresarial em conta nenhuma: quem decide se a Lia o chama é
...
class AddEmpresarialSpecialistToQuoteAgents < ActiveRecord::Migration[7.2]
  SLUG = 'cotacao_empresarial'.freeze
